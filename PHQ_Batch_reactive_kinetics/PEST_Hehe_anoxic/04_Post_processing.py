...
def plot_prior_vs_glue_subplots(prior_df, posterior, glue_df, output_dir, title="Prior vs GLUE"):
    ...
    # =========================
    exclude_params = {
        "k_sdz", "k_sdx", "k15", "k11", "k14", "k2", "k9", "k1", "k_doc2",  "k_no3"
    }
    ...
    for i, col in enumerate(cols):
        ...
        ax.hist(prior[col].dropna(), bins=30, alpha=0.5, label="prior", color="grey")
    ...
    out_file = os.path.join(output_dir, "FigS10_anoxic_H_PDF.png")
...
def main():
    ...
    if "k6" in posterior.columns and "k12" in posterior.columns and "k_doc" in posterior.columns:

        k6_log = np.log10(0.2 * posterior["k12"] +0.8*posterior["k_doc"] )
        k6_log = k6_log - 1.5   # decrease values
        k7_log = np.log10(0.8 * posterior["k_doc"]  )
        k7_log = k7_log + 10.5   # decrease values
        posterior["k6"] = 10 ** k6_log
        posterior["k7"] = 10 ** k7_log
