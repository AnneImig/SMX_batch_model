...
def plot_prior_vs_glue_subplots(prior_df, posterior, glue_df, output_dir, title="Prior vs GLUE"):
    ...
    # =========================
    exclude_params = {
        "k_sdz", "k_sdx", "k1", "k2",  "k8", "k_doc","k9","k10", "k12",  "k_doc2"
    }
    ...
    for i, col in enumerate(cols):
        ...
        # condition: first column AND first 3 rows
        if row == 0 :
            prior_color = "grey"
            glue_color = "lightblue"
            ax.hist(prior[col].dropna(), bins=30, alpha=0.5,
                label="prior", color=prior_color)
            ax.hist(glue[col].dropna(), bins=30, alpha=0.7,
                    label="posterior", color=glue_color)
        ...
        if col.startswith("k_no3"):
            suffix = col.replace("k_no3", "")
            label = rf"$k_{{NO_3^-}}{suffix}$"
