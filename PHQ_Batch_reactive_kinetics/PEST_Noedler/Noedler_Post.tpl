ptf $
Title Noedler Batch experiment
...
temp    	25
N(+5)		6.7E-2
Ca	    	5.892821523e-3 charge
Alkalinity 	7e-3 as HCO3-
Doc 		7.15e-2
Amonia   	1e-5 as AmmH+ # to incorporate in biomass
Cl      	1.43e-3
#P       	1.77e-3 as PO4-2
#S       	1.35e-3 as SO4-2
Mg      	1.38e-3
...
Som
    ...
    -m0 2e-3 #0.0049g/10ml TOC CH4O 32 mol/L 0.015 mol/L 15% solved in water 2.3e-3 mol/L DOC?
       -parms    $K1               $
    -steps  0 900*0.1          #90 days in 900 time steps?
...
Doc_Degradation_NITdos
    -formula  Doc  -15 Nit- -8 N2 +4 C+4 6
    # (1) K7 (2)  inhib_NO3
       -parms    $K7     $  $K_NO3    $ 
...
-start
    #10 HNO3= mol('H+')
    11 HNO3= (mol("NO3-")*mol("H+"))/2.4e-4
    20 rate= parm(1)*tot("Des")*mol("NO3-")
    25 put(rate, 210)
	30 moles=rate*time
	40 SAVE moles
-end
...
SELECTED_OUTPUT 1
       -file   /Users/anneimig/Library/Mobile Documents/com~apple~CloudDocs/02_SUSTech/02_Column Study Yunjie/03_Model/Pestim_Column/PHQ_Batch_reactive_kinetics/PEST_Noedler_log/output/Results.sel
	#-distance true
	-time true
	-reset false
...
90 PUNCH tot('Smx')  tot('Sdz') tot('Smz') tot("Ammet") tot('Des') get(16) get(17) get(18) get(19) tot("Nitro")*0.7 get(20) get(21)
...
150 PUNCH get(191) MOL("CO2") TOT("Label") get(210) get(800) tot('Undet') get(990) get(900) TOT('Alc')  get(999)
  	-end
